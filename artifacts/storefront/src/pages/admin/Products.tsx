      ...
      <div className="p-6 md:p-8">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-2">
                  ...
                  <FormField control={form.control} name="images" render={({ field }) => (
                    <FormItem><FormLabel>Image URLs (one per line)</FormLabel><FormControl><Textarea {...field} rows={2} placeholder="https://..." /></FormControl></FormItem>
                  )} />
        ...
        <div className="flex gap-3 mb-6">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && setSearchQ(search)} placeholder="Search products..." className="pl-9" />
          </div>
          <Button variant="outline" onClick={() => setSearchQ(search)}>Search</Button>
        </div>

        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="hidden md:grid grid-cols-[2fr,1fr,1fr,1fr,80px] gap-4 px-5 py-3 border-b border-border text-xs font-medium text-muted-foreground uppercase tracking-wide">
            <span>Product</span><span>Category</span><span>Price</span><span>Stock</span><span />
          </div>
          {isLoading ? (
            ...
            data.items.map((product, i) => (
              <div key={product.id} className={`grid grid-cols-[1fr,80px] md:grid-cols-[2fr,1fr,1fr,1fr,80px] gap-4 px-5 py-4 items-center ${i !== data.items.length - 1 ? "border-b border-border" : ""}`} data-testid={`product-row-${product.id}`}>
                <div className="flex items-center gap-3 min-w-0">
                  {product.images[0] && (
                    <img src={product.images[0]} alt={product.name} className="h-10 w-10 rounded-lg object-cover flex-shrink-0 bg-muted" />
                  )}
                ...
                <span className="hidden md:block text-sm font-medium text-foreground">${product.price.toFixed(2)}</span>
                ...
                <div className="flex gap-1 justify-end">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" data-testid={`button-edit-${product.id}`}>
                    <Pencil className="h-3.5 w-3.5" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => handleDelete(product.id, product.name)} data-testid={`button-delete-${product.id}`}>
